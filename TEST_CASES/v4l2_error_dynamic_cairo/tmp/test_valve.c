...
/*
  ...
  | Camera  |  | Video |  | Caps   |  | Valve |  | Video   |  | Video |
  | v4l2src |->| Rate  |->| Filter |->|       |->| convert |->| sink  |-> Screen

    but is dynamically changed to:-

  | Camera  |  | Video |  | Caps   |  | Valve |  | Video   |  | Cairo   |  | Video   | | Video |
  | v4l2src |->| Rate  |->| Filter |->|       |->| convert |->| overlay |->| convert | | sink  |-> Screen
...
{
    GstElement *pipeline;
    GstElement *v_convert, *v_sink, *c_valve;
    GstElement *cairo_overlay, *cairo_convert;
...
int prepare_reticule(AppData *appdata)
{
    ...
    /* Unlink from v_sink */
    g_object_set (appdata->c_valve, "drop", TRUE, NULL);
    ...
    */
    ...
    g_object_set (appdata->c_valve, "drop", FALSE, NULL);
    return TRUE;
}
...
int process_ret (gpointer user_data)
{
    AppData *appdata;
	...
	prepare_reticule(appdata);
    else
	remove_reticule(appdata);
...
{
    GstElement *pipeline, *v4l2_src, *v_rate, *v_filter;
    GstCaps *v_caps;
    GstBus *bus;
    ...
    appdata->c_valve = gst_element_factory_make ("valve", "block");
    ...
    /* Check elements */
    if (! (pipeline && v4l2_src && v_rate && v_filter && appdata->c_valve && 
    	               appdata->v_convert && appdata->v_sink))
    {
    ...
    gst_bin_add_many (GST_BIN (pipeline), v4l2_src, v_rate, v_filter, 
    					  appdata->c_valve, appdata->v_convert, appdata->v_sink, NULL);

    if (gst_element_link_many (v4l2_src, v_rate, v_filter, 
    			       appdata->c_valve, appdata->v_convert, appdata->v_sink, NULL) != TRUE)
    {
